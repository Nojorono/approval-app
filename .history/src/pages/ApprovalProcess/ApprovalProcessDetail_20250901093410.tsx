...
export default function ApprovalProcessDetail() {
  ...
  const handleSubmit = async (data: ApprovalProcess): Promise<{ success: boolean; error?: string }> => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      if (response?.success) {
        ...
        navigate("/Approval-Process/result", { state: { data: { ...data, createdAt: Date.now(), creator: data.creator.username } } });
        return { success: true };
      } else {
        alert(response?.message || "Gagal submit!");
        return { success: false, error: response?.message || "Gagal submit!" };
      }
    } catch (err: any) {
